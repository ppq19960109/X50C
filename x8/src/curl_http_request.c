...
static char http_report_hex_buf[2500];
...
void http_report_hex(char *title, const unsigned char *buff, int buff_len)
{
    ...
    http_report_hex_len += hex_to_str(buff, buff_len, http_report_hex_buf + http_report_hex_len, sizeof(http_report_hex_buf) - http_report_hex_len);
    if (http_report_hex_len > 1800)
    {
