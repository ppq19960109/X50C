...
static void *UpdateHistory_cb(void *ptr, void *arg)
{
    ...
    ++g_history_seqid;
    dzlog_info("UpdateHistory_cb:g_history_seqid:%d\n", g_history_seqid);
    return root;
}
...
static void *InsertHistory_cb(void *ptr, void *arg)
{
    ...
    {
        recipe.recipeType = cJSON_GetObjectItem(item, "recipeType")->valueint;
    }
    if (cJSON_HasObjectItem(item, "cookPos"))
    {
        recipe.cookPos = cJSON_GetObjectItem(item, "cookPos")->valueint;
    }
